export async function handler(event, context) {
  const VERIFY_TOKEN = "YOUR_VERIFY_TOKEN"; // ضع رمز التحقق اللي اخترته
  const PAGE_ACCESS_TOKEN = "YOUR_PAGE_ACCESS_TOKEN"; // انسخه من فيسبوك
    ...
    if (body.object === "page") {
      for (const entry of body.entry) {
        ...
        if (event.message && event.message.text) {
          ...
          // ✅ الردود الخاصة بخدمة عملاء موبليس
